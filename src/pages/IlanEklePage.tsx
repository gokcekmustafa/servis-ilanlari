                    ...
                    {aracFormAcik && (
                      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                          <div><label className={lb}>Marka</label><select className={ic} value={yeniAracForm.marka} onChange={(e)=>setYeniAracForm({...yeniAracForm,marka:e.target.value})}><option value="">Secin</option>{['Mercedes','Ford','Volkswagen','Renault','Peugeot','Citroen','Iveco','Temsa','Isuzu'].map(m=><option key={m} value={m}>{m}</option>)}</select></div>
                          <div><label className={lb}>Model</label><input className={ic} value={yeniAracForm.model} placeholder="Sprinter, Transit..." onChange={(e)=>setYeniAracForm({...yeniAracForm,model:e.target.value})}/></div>
                ...
                {selectedKategori === 'aracimi_satiyorum' && (
                  <div className="border border-slate-200 rounded-xl p-4 md:p-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                      <div><label className={lb}>Marka</label><select value={aracimiSatiyorum.marka} onChange={(e)=>setAracimiSatiyorum({...aracimiSatiyorum,marka:e.target.value})} className={ic}><option value="">Secin</option>{['Mercedes','Ford','Volkswagen','Renault','Peugeot','Citroen','Iveco','Temsa','Isuzu'].map(m=><option key={m} value={m}>{m}</option>)}</select></div>
                      <div><label className={lb}>Model</label><input value={aracimiSatiyorum.model} placeholder="Sprinter, Transit..." onChange={(e)=>setAracimiSatiyorum({...aracimiSatiyorum,model:e.target.value})} className={ic}/></div>
