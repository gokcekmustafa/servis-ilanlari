...
export default function IlanEklePage({
  ...
  const handleServisTuru = (tur: string) => {
    if (form.servisTuru.includes(tur)) {
      setForm({ ...form, servisTuru: form.servisTuru.filter((t) => t !== tur) });
    } else {
  ...
  const handleAdim2 = () => {
    if (!form.aciklama || !form.kalkis_il || !form.varis_il || !form.ilan_veren) {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            <React.Fragment key={s}>
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition ${
                  adim > s ? 'bg-green-500 text-white' : adim === s ? 'bg-[#1a3c6e] text-white' : 'bg-gray-200 text-gray-500'
                }`}>
              ...
              {kategoriler.map((kat) => (
                <button
                  ...
                  className={`border-2 rounded-xl p-4 text-sm font-medium text-left transition ${
                    selectedKategori === kat.id ? kat.color : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                  }`}
                >
                  {kat.label}
                </button>
        ...
        {adim === 2 && (
          <div className="flex flex-col gap-4">
            <h3 className="font-semibold text-gray-700">Ilan detaylarini girin</h3>

            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">
                İlan Veren <span className="text-red-500">*</span>
              </label>
              <input
                value={form.ilan_veren}
                onChange={(e) => setForm({ ...form, ilan_veren: e.target.value })}
                placeholder="Firma veya adiniz"
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 mb-2 block">Servis Turu</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Giris Saati</label>
                <input
                  type="time"
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Cikis Saati</label>
                <input
                  type="time"
            ...
            <div className="border border-gray-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-gray-600 mb-3">
                Kalkis Bilgileri <span className="text-red-500">*</span>
              </p>
            ...
            <div className="border border-gray-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-gray-600 mb-3">
                Varis Bilgileri <span className="text-red-500">*</span>
              </p>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Il</label>
        ...
        {adim === 3 && (
          <div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-6">
              ...
              <p className="text-sm text-gray-700 mb-2 font-medium">{form.ilan_veren}</p>
              ...
              <table className="w-full text-xs border border-gray-200 rounded">
                ...
                <tbody>
                  <tr className="bg-white">
                    <td className="px-2 py-1 font-medium text-[#1a3c6e]">{form.giris_saati || '--:--'}</td>
                    <td className="px-2 py-1">{form.kalkis_mah} {form.kalkis_ilce} / {form.kalkis_il}</td>
                    <td className="px-2 py-1">{form.varis_mah} {form.varis_ilce} / {form.varis_il}</td>
                    <td className="px-2 py-1 font-medium text-[#1a3c6e]">{form.cikis_saati || '--:--'}</td>
                  </tr>
                </tbody>
              </table>
              {form.servisTuru.length > 0 && (
                <p className="text-xs text-gray-500 mt-3">Servis Turu: {form.servisTuru.join(', ')}</p>
              )}
            </div>
