...
// â”€â”€â”€ TÄ°PLER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const PERSONEL_YETKI_TANIM: Record<string, { label: string; aciklama: string }> = {
  ilan_onay:          { label: 'Ä°lan Onay',   aciklama: 'Ä°lanlarÄ± aktif/pasif yapabilir' },
  ilan_sil:           { label: 'Ä°lan Sil',    aciklama: 'Ä°lanlarÄ± silebilir' },
  kullanici_yonetimi: { label: 'KullanÄ±cÄ±',  aciklama: 'KullanÄ±cÄ±larÄ± dÃ¼zenleyip yÃ¶netebilir' },
  destek_yonetimi:    { label: 'Destek',     aciklama: 'Destek taleplerini cevaplayabilir' },
  reklam_yonetimi:    { label: 'Reklam',     aciklama: 'Reklam ekleyip yÃ¶netebilir' },
  duyuru_yonetimi:    { label: 'Duyuru',     aciklama: 'Duyuru ekleyip yÃ¶netebilir' },
};

const KISITLAMA_TANIM: Record<string, { label: string; aciklama: string; ikon: React.ReactNode }> = {
  mesaj_gonderme:          { label: 'MesajlaÅŸma',              aciklama: 'KullanÄ±cÄ± mesaj gÃ¶nderemez',             ikon: <MessageCircle size={14} /> },
  profil_resmi_degistirme: { label: 'Profil Resmi DeÄŸiÅŸtirme', aciklama: 'Profil fotoÄŸrafÄ±nÄ± deÄŸiÅŸtiremez',        ikon: <Camera size={14} /> },
  profil_resmi_silme:      { label: 'Profil Resmi Silme',      aciklama: 'Profil fotoÄŸrafÄ±nÄ± silemez',             ikon: <Trash2 size={14} /> },
  ilan_verme:              { label: 'Ä°lan Verme',              aciklama: 'Yeni ilan oluÅŸturamaz',                  ikon: <FileText size={14} /> },
  yorum_yapma:             { label: 'Yorum Yapma',             aciklama: 'Yorum ve deÄŸerlendirme yapamaz',         ikon: <Star size={14} /> },
  arama:                   { label: 'Arama',                   aciklama: 'Arama Ã¶zelliÄŸini kullanamaz',            ikon: <Ban size={14} /> },
  favori_ekleme:           { label: 'Favori Ekleme',           aciklama: 'Ä°lan favorilere ekleyemez',              ikon: <Star size={14} /> },
  bildirim_alma:           { label: 'Bildirimler',             aciklama: 'Uygulama bildirimleri iletilmez',        ikon: <Bell size={14} /> },
  profil_goruntuleme:      { label: 'Profil Gizleme',          aciklama: 'Profili diÄŸer kullanÄ±cÄ±lara gizlenir',   ikon: <Eye size={14} /> },
  iletisim_bilgisi:        { label: 'Ä°letiÅŸim Bilgisi',        aciklama: 'BaÅŸkalarÄ± iletiÅŸim bilgisini gÃ¶remez',   ikon: <Phone size={14} /> },
  dosya_yukleme:           { label: 'Dosya YÃ¼kleme',           aciklama: 'Dosya ve resim yÃ¼kleyemez',              ikon: <Download size={14} /> },
  hesap_silme:             { label: 'Hesap Silme',             aciklama: 'Kendi hesabÄ±nÄ± silemez',                 ikon: <AlertTriangle size={14} /> },
};

// â”€â”€â”€ ANA COMPONENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const [logoYukleniyor, setLogoYukleniyor] = useState(false);
  ...
  // Reklam dÃ¼zenleme state'leri
  const [duzenleReklam, setDuzenleReklam]       = useState<any>(null); // dÃ¼zenlenen reklam
  ...
  const [yeniKullaniciDuyuru, setYeniKullaniciDuyuru] = useState({ baslik: '', mesaj: '' });
  ...
  const hepsiniYukle = async () => {
    ...
    setIlanlar(i.data || []);
    setReklamlar(r.data || []);
    let popupDuyurular = d.data || [];
    const aktifPopupDuyurular = popupDuyurular.filter((item: any) => item?.aktif);
    if (aktifPopupDuyurular.length > 1) {
      const kapanacaklar = aktifPopupDuyurular.slice(1).map((item: any) => item.id);
      if (kapanacaklar.length > 0) {
        await supabase.from('duyurular').update({ aktif: false }).in('id', kapanacaklar);
        const kapanacakSet = new Set(kapanacaklar);
        popupDuyurular = popupDuyurular.map((item: any) =>
          kapanacakSet.has(item.id) ? { ...item, aktif: false } : item
        );
      }
    }
    setDuyurular(popupDuyurular);
  ...
  // â”€â”€â”€ KULLANICI DETAY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Ä°LAN â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ REKLAM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const ilanAktifSureKaydet = async () => {
    ...
    if (pasifError) {
      setIlanSureMesaj('SÃ¼re kaydedildi, sÃ¼re dolan ilanlar gÃ¼ncellenemedi.');
      setTimeout(() => setIlanSureMesaj(''), 3500);
      return;
    }

    setIlanSureMesaj(`Kaydedildi âœ“ (${yeniSure} gÃ¼n)`);
  ...
  const adminIlanDurumDegistir = async (ilan: any) => {
    ...
    const onayMesaji = hedefDurum === 'aktif'
      ? `Bu ilanÄ± tekrar aktif yapmak istiyor musunuz? SayaÃ§ ${ilanAktifSureGun} gÃ¼ne sÄ±fÄ±rlanacak.`
      : 'Bu ilanÄ± pasif yapmak istiyor musunuz?';

    if (!window.confirm(onayMesaji)) return;

    const { error } = await ilanDurumGuncelle(ilan.id, hedefDurum, ilan.ekbilgiler);
    if (error) {
      alert('Ä°lan durumu gÃ¼ncellenemedi: ' + error.message);
      return;
    }
  ...
  const dosyaYukle = async (dosya: File, hedef: 'yeni' | 'duzenle' = 'yeni') => {
    if (!dosya.type.startsWith('image/')) { alert('Sadece resim dosyasÄ± yÃ¼kleyebilirsiniz'); return; }
    ...
    if (error) { alert('YÃ¼klerken hata: ' + error.message); setReklamYukleniyor(false); setDuzenleResimYukleniyor(false); return; }
  ...
  const siklikKaydet = async () => {
    ...
    // upsert: anahtar yoksa ekle, varsa gÃ¼ncelle
  ...
  const duyuruResimYukle = async (dosya: File) => {
    ...
    const { error } = await supabase.storage.from('reklamlar').upload(ad, dosya, { contentType: dosya.type });
    if (error) { alert('YÃ¼klerken hata: ' + error.message); setDuyuruYukleniyor(false); return; }
    const { data: u } = supabase.storage.from('reklamlar').getPublicUrl(ad);
    setYeniDuyuru(p => ({ ...p, resim_url: u.publicUrl }));
    setDuyuruYukleniyor(false);
  };

  const popupAktifliginiGuncelle = async (duyuruId: any, aktiflestir: boolean) => {
    if (aktiflestir) {
      await supabase.from('duyurular').update({ aktif: false }).neq('id', duyuruId);
      await supabase.from('duyurular').update({ aktif: true }).eq('id', duyuruId);
      return;
    }
    await supabase.from('duyurular').update({ aktif: false }).eq('id', duyuruId);
  };

  // â”€â”€â”€ PERSONEL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const personelKaydet = async () => {
    ...
    if (seciliPersonel) {
      ...
      if (!personelForm.password) { alert('Yeni personel iÃ§in ÅŸifre zorunludur.'); return; }
  ...
  const firmaListesiniKaydet = async () => {
    ...
    localStorage.setItem('kurumsal_firma_listesi_guncelleme', String(Date.now()));
    window.dispatchEvent(new Event('kurumsal-firmalar:degisti'));
    setTimeout(() => setFirmaMesaj(''), 2500);
  };

  // â”€â”€â”€ STILLER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const YetkisizUyari = ({ sekme }: { sekme: string }) => (
    <div className="flex flex-col items-center justify-center py-16 sm:py-24 text-slate-400">
      ...
      <p className="text-sm font-medium text-slate-500">Bu bÃ¶lÃ¼me eriÅŸim yetkiniz yok</p>
      <p className="text-xs mt-1 text-center px-4">YÃ¶neticinizden "{sekme}" yetkisi talep ediniz.</p>
    </div>
  );
  ...
  const { error } = await supabase.storage.from('reklamlar').upload(ad, dosya, { contentType: dosya.type });
  if (error) { alert('YÃ¼klerken hata: ' + error.message); setLogoYukleniyor(false); return; }
  const { data: u } = supabase.storage.from('reklamlar').getPublicUrl(ad);
      ...
      {!isSuperAdmin && (
        <div className="px-4 py-3 border-t border-slate-700">
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {Object.values(yetkiler).every(v => !v) && <span className="text-[10px] text-slate-500">Yetki atanmamÄ±ÅŸ</span>}
          </div>
        </div>
      )}
    ...
    <div className="flex min-h-screen bg-slate-100">
      ...
      <main className="flex-1 overflow-auto pt-14 lg:pt-0">
        <div className="p-3 sm:p-4 lg:p-6">
          ...
          {yukleniyor && <div className="text-center py-20 text-slate-400 text-sm">YÃ¼kleniyor...</div>}
          ...
          {!yukleniyor && aktifSekme === 'istatistik' && (
            <div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
                {[
                  { label: 'Toplam Ä°lan',     value: ilanlar.length,                                       renk: 'bg-blue-50 text-blue-700 border-blue-200' },
                  { label: 'Toplam Ãœye',      value: kullanicilar.filter(u => u.type !== 'admin').length,  renk: 'bg-green-50 text-green-700 border-green-200' },
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Son Ä°lanlar</p>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Son Ãœyeler</p>
                  {kullanicilar.filter(u => u.type !== 'admin').slice(0, 5).map(u => (
                    <div key={u.id} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                      <span className="text-sm text-slate-600 truncate mr-2">{u.full_name || 'Ä°simsiz'}</span>
                      <span className="text-xs text-slate-400 flex-shrink-0">{u.phone_number}</span>
                    </div>
                  ))}
                </div>
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Ã‡evrimiÃ§i KullanÄ±cÄ±lar</p>
                  <div className="max-h-64 overflow-y-auto pr-1">
                    ...
                    {cevrimiciKullanicilar.length === 0 && <p className="text-xs text-slate-400 py-2">Åu an Ã§evrimiÃ§i kullanÄ±cÄ± yok.</p>}
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  <p className="text-xs text-slate-400 mb-1">Toplam Ä°lan GÃ¶rÃ¼ntÃ¼leme</p>
                  <p className="text-xl font-bold text-slate-700">{toplamGoruntulenme.toLocaleString('tr-TR')}</p>
                </div>
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  <p className="text-xs text-slate-400 mb-1">Ãœye EtkileÅŸimi (Favori)</p>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  <p className="text-xs text-slate-400 mb-1">Ã‡evrimiÃ§i KullanÄ±cÄ±</p>
          ...
          {!yukleniyor && aktifSekme === 'kullanicilar' && (
            !sekmeYetkiVarMi('kullanicilar') ? <YetkisizUyari sekme="KullanÄ±cÄ± YÃ¶netimi" /> : (
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="sm:hidden divide-y divide-slate-100">
                  ...
                  return (
                    <div key={u.id} onClick={() => kullaniciDetayAc(u)} className="p-4 flex items-center gap-3 active:bg-slate-50">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-slate-700 font-medium text-sm truncate">{u.full_name || 'Ä°simsiz'}</p>
                      ...
                      <div className="flex flex-col items-end gap-1 flex-shrink-0">
                        ...
                        {ks > 0 && <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">{ks} kÄ±sÄ±tlama</span>}
                      </div>
                ...
                {kullanicilar.filter(u => u.type !== 'admin').length === 0 && (
                  <div className="text-center py-12 text-slate-400 text-sm">HenÃ¼z Ã¼ye yok</div>
                )}
              </div>
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {['Ad Soyad', 'Telefon', 'Tip', 'Durum', 'KÄ±sÄ±tlama', 'KayÄ±t Tarihi', 'Ä°ÅŸlem'].map(h => (
                    ...
                    {kullanicilar.filter(u => u.type !== 'admin').map(u => {
                      ...
                      return (
                        <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50 transition cursor-pointer" onClick={() => kullaniciDetayAc(u)}>
                          <td className="px-4 py-3"><div className="font-medium text-slate-700">{u.full_name || 'Ä°simsiz'}</div>{u.email && <div className="text-xs text-slate-400">{u.email}</div>}</td>
                          <td className="px-4 py-3 text-slate-500 text-xs">{u.phone_number}</td>
                          <td className="px-4 py-3"><span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{u.type === 'admin' ? 'YÃ¶netici' : 'Ãœye'}</span></td>
                          ...
                          <td className="px-4 py-3">{ks > 0 ? <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">{ks} kÄ±sÄ±tlama</span> : <span className="text-xs text-slate-300">â€”</span>}</td>
                          <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{u.created_at ? new Date(u.created_at).toLocaleDateString('tr-TR') : 'â€”'}</td>
                ...
                {kullanicilar.filter(u => u.type !== 'admin').length === 0 && <div className="text-center py-12 text-slate-400 text-sm">HenÃ¼z Ã¼ye yok</div>}
              </div>
            </div>
            )
          )}

          {/* â”€â”€â”€ Ä°LANLAR â”€â”€â”€ */}
          {!yukleniyor && aktifSekme === 'ilanlar' && (
            !sekmeYetkiVarMi('ilanlar') ? <YetkisizUyari sekme="Ä°lan Onay" /> : (
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/70">
                <p className="text-xs font-semibold text-slate-600 mb-2">Ä°lanlarÄ±n aktif kalma sÃ¼resi (gÃ¼n)</p>
              ...
              <div className="sm:hidden divide-y divide-slate-100">
                  ...
                  <div key={ilan.id} className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{ilan.kategori} Â· {new Date(ilan.created_at).toLocaleDateString('tr-TR')}</p>
                      </div>
                    ...
                    {ilan.durum === 'aktif' && (
                      <p className="text-[11px] text-amber-700 mb-2">{ilanKalanGun(ilan)} gÃ¼n kaldÄ±</p>
                    )}
                ...
                {ilanlar.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">HiÃ§ ilan yok</div>}
              </div>
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {['Ä°lan Veren', 'Kategori', 'Tarih', 'Durum', 'Ä°ÅŸlemler'].map(h => (
                        <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
                      ...
                      <tr key={ilan.id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1">
                            ...
                            {ilan.durum === 'aktif' && (
                              <span className="text-[11px] text-amber-700">{ilanKalanGun(ilan)} gÃ¼n kaldÄ±</span>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
          ...
          {/* â”€â”€â”€ REKLAMLAR â”€â”€â”€ */}
          {!yukleniyor && aktifSekme === 'reklamlar' && (
            !sekmeYetkiVarMi('reklamlar') ? <YetkisizUyari sekme="Reklam YÃ¶netimi" /> : (
            <div className="flex flex-col gap-4">

              {/* Ä°lan ArasÄ± Reklam SÄ±klÄ±ÄŸÄ± AyarÄ± */}
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                <p className="text-sm font-semibold text-slate-700 mb-1 flex items-center gap-1.5">
                  <Image size={15} className="text-orange-500" />
                  Ä°lan ArasÄ± Reklam SÄ±klÄ±ÄŸÄ±
                </p>
                <p className="text-xs text-slate-400 mb-3">Her kaÃ§ ilandan sonra liste iÃ§i reklam gÃ¶sterilsin?</p>
              ...
              <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2 flex flex-col gap-3">
                  {reklamlar.map(r => (
                    <div key={r.id} className="bg-white rounded-xl border border-slate-200 p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-slate-700 text-sm">{r.baslik || 'BaÅŸlÄ±ksÄ±z'}</p>
                          ...
                          <span className={'text-xs font-semibold px-2 py-0.5 rounded-full mt-1 inline-block ' + (r.konum === 'header' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500')}>
                            {r.konum === 'header' ? 'Ãœst Alan' : r.konum === 'kenar_kucuk' ? 'Yan - KÃ¼Ã§Ã¼k' : r.konum === 'kenar_buyuk' ? 'Yan - BÃ¼yÃ¼k' : 'Liste ArasÄ±'}
                          </span>
                        </div>
                        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 flex-shrink-0">
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-orange-500 transition" title="DÃ¼zenle">
                            <Edit2 size={15} />
                          </button>
                          <button
                            onClick={async () => { if (!window.confirm('Bu reklamÄ± silmek istiyor musunuz?')) return; await supabase.from('reklamlar').delete().eq('id', r.id); hepsiniYukle(); }}
                  ...
                  {reklamlar.length === 0 && (
                    <div className="bg-white rounded-xl border border-slate-200 py-12 text-center text-slate-400 text-sm">HiÃ§ reklam eklenmemiÅŸ</div>
                  )}
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  ...
                  <input className={ic + ' mb-2'} placeholder="BaÅŸlÄ±k (opsiyonel)" value={yeniReklam.baslik} onChange={e => setYeniReklam({ ...yeniReklam, baslik: e.target.value })} />
                  <input className={ic + ' mb-2'} placeholder="TÄ±klama linki" value={yeniReklam.link_url} onChange={e => setYeniReklam({ ...yeniReklam, link_url: e.target.value })} />
                  <select className={ic + ' mb-3'} value={yeniReklam.konum} onChange={e => setYeniReklam({ ...yeniReklam, konum: e.target.value })}>
                    <option value="liste">Liste ArasÄ±</option>
                    <option value="header">Ãœst Alan</option>
                    <option value="kenar_kucuk">Yan Alan - KÃ¼Ã§Ã¼k (8cm)</option>
                    <option value="kenar_buyuk">Yan Alan - BÃ¼yÃ¼k (12cm)</option>
                    <option value="kenar_sol">Dikey Sol Kenar</option>
                    <option value="kenar_sag">Dikey SaÄŸ Kenar</option>
                  </select>
                  {yeniReklam.resim_url ? (
                    <div className="mb-3 relative">
                      ...
                      <button onClick={() => setYeniReklam({ ...yeniReklam, resim_url: '' })} className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">Ã—</button>
                      <p className="text-xs text-green-600 mt-1 font-medium">Resim yÃ¼klendi âœ“</p>
                    </div>
                  ) : (
                    <div onDragOver={e => { e.preventDefault(); setSurukleAktif(true); }} onDragLeave={() => setSurukleAktif(false)}
                      ...
                      {reklamYukleniyor
                        ? <div className="flex flex-col items-center gap-2"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /><p className="text-xs text-slate-400">YÃ¼kleniyor...</p></div>
                        : <div className="flex flex-col items-center gap-2"><Image size={24} className="text-slate-300" /><p className="text-sm text-slate-500">SÃ¼rÃ¼kle bÄ±rak veya tÄ±kla</p><p className="text-xs text-slate-400">PNG, JPG, GIF</p></div>}
          ...
          {!yukleniyor && aktifSekme === 'popup' && (
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 flex flex-col gap-3">
                  ...
                  <div key={d.id} className="bg-white rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-slate-700 text-sm">{d.baslik || 'Basliksiz popup mesaji'}</p>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          onClick={async () => { await popupAktifliginiGuncelle(d.id, !d.aktif); hepsiniYukle(); }}
                ...
                {duyurular.length === 0 && <div className="bg-white rounded-xl border border-slate-200 py-12 text-center text-slate-400 text-sm">Hic popup mesaji eklenmemis</div>}
              </div>
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                <p className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-1.5"><PlusCircle size={15} className="text-orange-500" />Yeni Popup Mesaji</p>
                <input className={ic + ' mb-2'} placeholder="Baslik (opsiyonel)" value={yeniDuyuru.baslik} onChange={e => setYeniDuyuru({ ...yeniDuyuru, baslik: e.target.value })} />
                ...
                {yeniDuyuru.resim_url ? (
                  ...
                  <div onDragOver={e => { e.preventDefault(); setDuyuruSurukle(true); }} onDragLeave={() => setDuyuruSurukle(false)}
                    ...
                    {duyuruYukleniyor
                      ? <div className="flex flex-col items-center gap-2"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /><p className="text-xs text-slate-400">Yukleniyor...</p></div>
                      : <div className="flex flex-col items-center gap-1.5"><Image size={22} className="text-slate-300" /><p className="text-sm text-slate-500 font-medium">Surukle birak veya tikla</p><p className="text-xs text-slate-400">PNG, JPG, GIF · Maks 5MB</p></div>}
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">Kac sn. sonra acilsin</label>
                  ...
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">Kac sn. gorunur kalsin</label>
                ...
                <button onClick={async () => {
                  ...
                  const { data: eklenen } = await supabase.from('duyurular').insert([{ ...yeniDuyuru, aktif: true }]).select('id').single();
                  if (eklenen?.id) {
                    await popupAktifliginiGuncelle(eklenen.id, true);
                  }
                  ...
                  className={btnO + ' w-full disabled:opacity-50 disabled:cursor-not-allowed'}>Popup Mesaji Ekle</button>
              </div>
            </div>
            )
          )}
          ...
          {!yukleniyor && aktifSekme === 'duyurular' && (
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4">
                  ...
                  <div key={d.id} className="bg-white rounded-xl border border-slate-200 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate-500 mt-1 whitespace-pre-wrap">{d.mesaj}</p>
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                ...
                <input className={ic + ' mb-2'} placeholder="Baslik" value={yeniKullaniciDuyuru.baslik} onChange={e => setYeniKullaniciDuyuru({ ...yeniKullaniciDuyuru, baslik: e.target.value })} />
                <textarea className={ic + ' mb-3 resize-none'} placeholder="Mesaj" rows={4} value={yeniKullaniciDuyuru.mesaj} onChange={e => setYeniKullaniciDuyuru({ ...yeniKullaniciDuyuru, mesaj: e.target.value })} />
                <button onClick={async () => {
                  const baslik = yeniKullaniciDuyuru.baslik.trim();
                  const mesaj = yeniKullaniciDuyuru.mesaj.trim();
                  if (!baslik || !mesaj) return;
                  await kullaniciDuyurusuYayinla({ baslik, mesaj });
                  setYeniKullaniciDuyuru({ baslik: '', mesaj: '' });
                  window.dispatchEvent(new Event('bildirimler:degisti'));
                  hepsiniYukle();
                  ...
                  <div key={d.id} onClick={async () => {
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={'flex-shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ' + (
                        ...
                        {d.durum === 'cevaplandi' ? 'CevaplandÄ±' : d.durum === 'islemde' ? 'Ä°ÅŸlemde' : 'Bekliyor'}
                      </span>
                    </div>
                  </div>
                ))}
                {destekTalepleri.length === 0 && <div className="bg-white rounded-xl border border-slate-200 py-12 text-center text-slate-400 text-sm">HiÃ§ destek talebi yok</div>}
              </div>
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                {seciliDestek ? (
                  <>
                    ...
                    <textarea className={ic + ' mb-3 resize-none'} placeholder="CevabÄ±n..." rows={5} value={destekCevap} onChange={e => setDestekCevap(e.target.value)} />
                ...
                ) : <p className="text-sm text-slate-400 text-center py-8">Detay iÃ§in bir destek talebi seÃ§</p>}
              </div>
            </div>
            )
          )}
          ...
          {!yukleniyor && aktifSekme === 'tavsiyeler' && (
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 flex flex-col gap-3">
                ...
                {tavsiyeKayitlari.length === 0 && <div className="bg-white rounded-xl border border-slate-200 py-12 text-center text-slate-400 text-sm">HenÃ¼z tavsiye yok</div>}
              </div>
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                {seciliTavsiye ? (
                  <>
                ...
                ) : <p className="text-sm text-slate-400 text-center py-8">Detay iÃ§in bir tavsiye seÃ§</p>}
              </div>
            </div>
            )
          )}
          ...
          {/* â”€â”€â”€ PERSONEL â”€â”€â”€ */}
          {!yukleniyor && aktifSekme === 'personel' && (
            !isSuperAdmin ? <YetkisizUyari sekme="Personel YÃ¶netimi" /> : (
            <div>
              ...
              {personelFormAcik && (
                <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-6 mb-4 sm:mb-6">
                  <h3 className="text-base font-bold text-slate-800 mb-4 border-b pb-2">{seciliPersonel ? 'Personel DÃ¼zenle' : 'Yeni Personel Ekle'}</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-5">
                    <div><label className="block text-xs font-medium text-slate-500 mb-1">Ad Soyad</label><input type="text" className={ic} value={personelForm.full_name} onChange={e => setPersonelForm({ ...personelForm, full_name: e.target.value })} placeholder="Ahmet YÄ±lmaz" /></div>
                    ...
                    <div><label className="block text-xs font-medium text-slate-500 mb-1">{seciliPersonel ? 'Yeni Åifre (boÅŸ = deÄŸiÅŸtirme)' : 'Åifre'}</label><input type="password" className={ic} value={personelForm.password} onChange={e => setPersonelForm({ ...personelForm, password: e.target.value })} placeholder="â€¢â€¢â€¢â€¢â€¢â€¢" /></div>
                  </div>
                  <div className="mb-5">
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Yetki AyarlarÄ±</label>
                    <p className="text-xs text-slate-400 mb-3">Personelin hangi bÃ¶lÃ¼mlere eriÅŸip iÅŸlem yapabileceÄŸini seÃ§in.</p>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 border-t border-slate-100">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setPersonelFormAcik(false); setSeciliPersonel(null); }} className={btnS + ' flex items-center gap-1 flex-1 sm:flex-none justify-center'}><X size={14} />Ä°ptal</button>
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <div className="sm:hidden divide-y divide-slate-100">
                  {kullanicilar.filter(u => u.type === 'admin').map(p => (
                    <div key={p.id} className="p-4">
                      ...
                      <div className="flex items-center gap-2 mb-2 bg-slate-50 px-3 py-2 rounded-lg">
                        <span className="font-mono text-xs text-slate-600 flex-1">{personelSifreGoster[p.id] ? (p.sifre_acik || '(kayÄ±tlÄ± deÄŸil)') : 'â€¢â€¢â€¢â€¢â€¢â€¢'}</span>
                      ...
                      <div className="flex gap-2 pt-1">
                        <button onClick={() => personelDuzenleAc(p)} className="flex-1 text-xs text-orange-500 border border-orange-200 bg-orange-50 py-1.5 rounded-lg flex items-center justify-center gap-1"><Edit2 size={12} />DÃ¼zenle</button>
                  ...
                  {kullanicilar.filter(u => u.type === 'admin').length === 0 && <div className="p-8 text-center text-slate-400 text-sm">HenÃ¼z personel eklenmemiÅŸ</div>}
                </div>
                <div className="hidden sm:block overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-xs text-slate-500 uppercase">
                        {['Personel', 'Telefon', 'Mevcut Åifre', 'Yetkiler', 'Durum', 'Ä°ÅŸlemler'].map(h => (
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {kullanicilar.filter(u => u.type === 'admin').map(p => (
                        <tr key={p.id} className="hover:bg-slate-50">
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-2">
                              <span className="font-mono text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded-lg">{personelSifreGoster[p.id] ? (p.sifre_acik || '(kayÄ±tlÄ± deÄŸil)') : 'â€¢â€¢â€¢â€¢â€¢â€¢'}</span>
                          ...
                          <td className="p-4">
                            <div className="flex gap-1">
                              <button onClick={() => personelDuzenleAc(p)} className="p-1.5 text-slate-400 hover:text-orange-500" title="DÃ¼zenle"><Edit2 size={14} /></button>
                      ...
                      {kullanicilar.filter(u => u.type === 'admin').length === 0 && (
                        <tr><td colSpan={6} className="p-8 text-center text-slate-400 text-sm">HenÃ¼z personel eklenmemiÅŸ</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          {/* â”€â”€â”€ LOGO YÃ–NETÄ°MÄ° â”€â”€â”€ */}
          {!yukleniyor && aktifSekme === 'logo' && (
            !isSuperAdmin ? <YetkisizUyari sekme="Logo YÃ¶netimi" /> : (
            <div className="max-w-lg">
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <p className="text-xs text-slate-400 mb-4">
                  YÃ¼klediÄŸiniz logo, sitenin header bÃ¶lÃ¼mÃ¼nde araÃ§ ikonu yerine gÃ¶rÃ¼necektir.
                </p>
                <div className="mb-4">
                ...
                >
                  {logoYukleniyor ? (
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-slate-500 font-medium">Logo yÃ¼klemek iÃ§in tÄ±kla</p>
                      <p className="text-xs text-slate-400">PNG, JPG, SVG Â· Åeffaf arka plan Ã¶nerilir</p>
                    </div>
                  )}
                ...
                {platformLogo && (
                  <p className="text-xs text-green-600 mt-3 font-medium text-center">âœ“ Logo aktif â€” header'da gÃ¶rÃ¼nÃ¼yor</p>
                )}
              </div>
            </div>
            )
      ...
      {/* â•â•â•â• REKLAM DÃœZENLEME MODAL â•â•â•â• */}
      {duzenleReklam && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
          onClick={e => { if (e.target === e.currentTarget) setDuzenleReklam(null); }}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <p className="font-bold text-slate-800 text-base flex items-center gap-2"><Edit2 size={16} className="text-orange-500" />ReklamÄ± DÃ¼zenle</p>
            ...
            <div className="p-5 space-y-3">
              <input className={ic} placeholder="BaÅŸlÄ±k (opsiyonel)" value={duzenleForm.baslik} onChange={e => setDuzenleForm({ ...duzenleForm, baslik: e.target.value })} />
              <input className={ic} placeholder="TÄ±klama linki" value={duzenleForm.link_url} onChange={e => setDuzenleForm({ ...duzenleForm, link_url: e.target.value })} />
              ...
              {/* Resim Ã¶nizleme + deÄŸiÅŸtirme */}
              <div>
                <p className="text-xs font-medium text-slate-500 mb-2">Reklam GÃ¶rseli</p>
                {duzenleForm.resim_url && (
                  <div className="relative mb-2 rounded-xl overflow-hidden border border-slate-200">
                    <img src={duzenleForm.resim_url} className="w-full h-32 object-cover" alt="Ã–nizleme" />
                    <button onClick={() => setDuzenleForm({ ...duzenleForm, resim_url: '' })}
                      className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold shadow hover:bg-red-600 transition">Ã—</button>
                  </div>
                )}
                <div
                  ...
                  {duzenleResimYukleniyor
                    ? <div className="flex items-center justify-center gap-2"><div className="w-5 h-5 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /><p className="text-xs text-slate-400">YÃ¼kleniyor...</p></div>
                    : <p className="text-xs text-slate-500">{duzenleForm.resim_url ? 'FarklÄ± bir resim seÃ§' : 'SÃ¼rÃ¼kle bÄ±rak veya tÄ±kla'}</p>}
            ...
            <div className="flex gap-3 p-5 border-t border-slate-100">
              <button onClick={() => setDuzenleReklam(null)} className={btnS + ' flex-1'}>Ä°ptal</button>
      ...
      {/* â•â•â•â• KULLANICI DETAY MODAL â•â•â•â• */}
      {detayModal && (
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-100 sticky top-0 bg-white z-10 rounded-t-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800 text-sm sm:text-base">{detayModal.full_name || 'Ä°simsiz KullanÄ±cÄ±'}</p>
            ...
            <div className="p-4 sm:p-5 space-y-5">
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                  {[
                    ...
                    { label: 'KullanÄ±cÄ± Tipi', value: detayModal.type === 'admin' ? 'YÃ¶netici' : 'Ãœye', alan: null, ikon: <Shield size={12} /> },
                    { label: 'KayÄ±t Tarihi', value: detayModal.created_at ? new Date(detayModal.created_at).toLocaleString('tr-TR') : 'â€”', alan: null, ikon: <Calendar size={12} /> },
                    { label: 'Son GiriÅŸ', value: detayModal.last_sign_in_at ? new Date(detayModal.last_sign_in_at).toLocaleString('tr-TR') : 'â€”', alan: null, ikon: <Calendar size={12} /> },
                  ].map(item => (
                    <div key={item.label} className="bg-slate-50 rounded-xl p-3">
                      ...
                      {detayDuzenle && item.alan
                        ...
                        : <p className="text-sm font-medium text-slate-700 break-all">{item.value || 'â€”'}</p>}
                    </div>
                  ))}
                </div>
              </div>
              <div>
                ...
                {detayDuzenle ? (
                  <div className="flex gap-2">
                    <button onClick={() => setDetayModal({ ...detayModal, aktif: true })} className={'flex-1 px-4 py-2.5 rounded-xl border text-sm font-medium transition ' + (detayModal.aktif !== false ? 'bg-green-500 text-white border-green-500' : 'text-slate-500 border-slate-200 bg-white')}>âœ“ Aktif</button>
                    <button onClick={() => setDetayModal({ ...detayModal, aktif: false })} className={'flex-1 px-4 py-2.5 rounded-xl border text-sm font-medium transition ' + (detayModal.aktif === false ? 'bg-red-500 text-white border-red-500' : 'text-slate-500 border-slate-200 bg-white')}>âœ• Pasif</button>
                  </div>
                ) : (
                  <span className={'inline-block text-sm font-semibold px-3 py-1.5 rounded-full ' + (detayModal.aktif !== false ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600')}>
                    {detayModal.aktif !== false ? 'âœ“ Aktif' : 'âœ• Pasif'}
                  </span>
                )}
              </div>
              {isSuperAdmin && (
                <div>
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Åifre Bilgisi</p>
                  <div className="bg-slate-50 rounded-xl p-4">
                    <div className="mb-3">
                      <p className="text-xs text-slate-400 mb-1.5">Mevcut Åifre</p>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-sm bg-white border border-slate-200 px-3 py-1.5 rounded-lg text-slate-700 flex-1 break-all">{detaySifreGoster ? (detayModal.sifre_acik || '(kayÄ±tlÄ± deÄŸil)') : 'â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢'}</span>
                    ...
                    {detayDuzenle && (
                      <div>
                        <label className="block text-xs text-slate-500 mb-1">Yeni Åifre Belirle</label>
                        <input type="text" className={ic} placeholder="Yeni ÅŸifre..." value={detaySifre} onChange={e => setDetaySifre(e.target.value)} />
                        <p className="text-xs text-slate-400 mt-1">BoÅŸ bÄ±rakÄ±rsan ÅŸifre deÄŸiÅŸmez.</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">KullanÄ±cÄ± KÄ±sÄ±tlamalarÄ±</p>
                ...
                {!detayDuzenle && Object.values(detayKisitlamalar).every(v => !v) && <p className="text-xs text-slate-400 italic mb-3">Bu kullanÄ±cÄ±ya hiÃ§bir kÄ±sÄ±tlama uygulanmamÄ±ÅŸ.</p>}
                  ...
                  {Object.entries(KISITLAMA_TANIM).map(([key, tanim]) => {
                    ...
                    return (
                      <label key={key} className={'flex items-center gap-3 p-3 border rounded-xl transition-colors ' + (detayDuzenle ? 'cursor-pointer ' : 'cursor-default ') + (aktif ? 'border-red-200 bg-red-50' : 'border-slate-100 bg-slate-50/50')}>
                        ...
                        {aktif && <span className="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-medium flex-shrink-0 whitespace-nowrap">KÄ±sÄ±tlÄ±</span>}
                      </label>
                    );
                  })}
                </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-4 sm:p-5 border-t border-slate-100 bg-slate-50 rounded-b-2xl sticky bottom-0">
              <button onClick={() => kullaniciSil(detayModal.id)} className={btnR + ' flex items-center justify-center gap-1.5 text-xs'}><Trash2 size={13} />HesabÄ± KalÄ±cÄ± Sil</button>
              {detayDuzenle && (
                <div className="flex gap-2">
                  <button onClick={() => { setDetayDuzenle(false); setDetaySifre(''); setDetayKisitlamalar(detayModal.kisitlamalar || {}); }} className={btnS + ' flex items-center gap-1 text-xs py-2 flex-1 sm:flex-none justify-center'}><X size={13} />Ä°ptal</button>
