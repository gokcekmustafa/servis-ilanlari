...
import {
  ...
  MessageCircle, Camera, Star, Bell, Ban, Download,
} from 'lucide-react';
...
export default function AdminPage({ onLogout, onIlanDetay, isSuperAdmin, yetkiler }: AdminPageProps) {
  ...
  const detayKaydet = async () => {
    ...
    const g: any = {
      full_name:    detayModal.full_name,
      phone_number: detayModal.phone_number,
      aktif:        detayModal.aktif,
      kisitlamalar: detayKisitlamalar,
    };
    if (detaySifre) {
      g.password_hash = await hashPassword(detaySifre);
      g.sifre_acik    = detaySifre;
    }
  ...
  const btnR = 'bg-red-50 hover:bg-red-100 text-red-600 text-sm font-medium px-4 py-2 rounded-lg transition border border-red-200';
  ...
  const YetkisizUyari = ({ sekme }: { sekme: string }) => (
    <div className="flex flex-col items-center justify-center py-24 text-slate-400">
      ...
      <p className="text-xs mt-1">Yöneticinizden "{sekme}" yetkisi talep ediniz.</p>
    </div>
  );
      ...
      <aside className="w-56 bg-slate-800 flex-shrink-0 flex flex-col">
        ...
        <nav className="flex-1 py-3">
          {menuItems.map(item => {
            ...
            return (
              <button key={item.id}
                onClick={() => { if (!item.kilitli) setAktifSekme(item.id as Sekme); }}
      ...
      <main className="flex-1 p-6 overflow-auto">
          ...
          <div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              {[
                ...
                <div key={s.label} className={'rounded-xl border p-4 ' + s.renk}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {kullanicilar.filter(u => u.type !== 'admin').map(u => {
                  const kisitSayisi = Object.values(u.kisitlamalar || {}).filter(Boolean).length;
                  return (
                    <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50 transition cursor-pointer" onClick={() => kullaniciDetayAc(u)}>
                      ...
                      <td className="px-4 py-3">
                        {kisitSayisi > 0
                          ? <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">{kisitSayisi} kısıtlama</span>
                          : <span className="text-xs text-slate-300">—</span>}
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-400">{u.created_at ? new Date(u.created_at).toLocaleDateString('tr-TR') : '—'}</td>
                      <td className="px-4 py-3">
                        <button onClick={e => { e.stopPropagation(); kullaniciDetayAc(u); }}
            ...
            <div className="col-span-2 flex flex-col gap-3">
              {reklamlar.map(r => (
                <div key={r.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-4">
                  <img src={r.resim_url} alt={r.baslik} className="w-32 h-16 object-cover rounded-lg border border-slate-100 flex-shrink-0"
        ...
        {!yukleniyor && aktifSekme === 'duyurular' && (
          ...
          <div className="grid grid-cols-3 gap-4">
              ...
              {duyurular.map(d => (
                <div key={d.id} className="bg-white rounded-xl border border-slate-200 p-4">
                  <div className="flex items-start justify-between">
                    <div><p className="font-semibold text-slate-700 text-sm">{d.baslik}</p><p className="text-xs text-slate-500 mt-1">{d.mesaj}</p><p className="text-xs text-slate-400 mt-1">{d.saniye} sn. sonra göster</p></div>
                    <div className="flex items-center gap-2 ml-4">
                      <button onClick={async () => { await supabase.from('duyurular').update({ aktif: !d.aktif }).eq('id', d.id); hepsiniYukle(); }}
                        className={'text-xs font-semibold px-3 py-1.5 rounded-lg border transition ' + (d.aktif ? 'bg-green-50 text-green-600 border-green-200' : 'bg-slate-50 text-slate-400 border-slate-200')}>
                        {d.aktif ? 'Aktif' : 'Pasif'}
                      </button>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 h-fit">
              ...
              <div className="flex items-center gap-2 mb-3"><label className="text-xs text-slate-500 flex-shrink-0">Gecikme (sn):</label><input className={ic} type="number" min={0} max={30} value={yeniDuyuru.saniye} onChange={e => setYeniDuyuru({ ...yeniDuyuru, saniye: Number(e.target.value) })} /></div>
        ...
        {!yukleniyor && aktifSekme === 'destek' && (
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2 flex flex-col gap-3">
              {destekler.map(d => (
                <div key={d.id} onClick={() => { setSeciliDestek(d); setDestekCevap(d.cevap || ''); }}
                  ...
                  <div className="flex items-start justify-between">
                    <div><p className="font-semibold text-slate-700 text-sm">{d.konu}</p><p className="text-xs text-slate-500 mt-1 line-clamp-2">{d.mesaj}</p></div>
                    <span className={'ml-4 flex-shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ' + (d.durum === 'cevaplandi' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700')}>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 h-fit">
              {seciliDestek ? (
                <><p className="text-sm font-semibold text-slate-700 mb-1">{seciliDestek.konu}</p><p className="text-xs text-slate-500 mb-3">{seciliDestek.mesaj}</p>
            ...
            {personelFormAcik && (
              <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                  <div><label className="block text-xs font-medium text-slate-500 mb-1">Ad Soyad</label><input type="text" className={ic} value={personelForm.full_name} onChange={e => setPersonelForm({ ...personelForm, full_name: e.target.value })} placeholder="Ör: Ahmet Yılmaz" /></div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setPersonelFormAcik(false); setSeciliPersonel(null); }} className={btnS + ' flex items-center gap-1'}><X size={14} />İptal</button>
                    <button onClick={personelKaydet} className={btnO + ' flex items-center gap-1'}><Save size={14} />Kaydet</button>
                  </div>
                </div>
              </div>
            )}
                ...
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-xs text-slate-500 uppercase">
                    {['Personel', 'Telefon', 'Mevcut Şifre', 'Yetkiler', 'Durum', 'İşlemler'].map(h => (
                      <th key={h} className="p-4 font-semibold text-left">{h}</th>
                    ))}
                  </tr>
                </thead>
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-1">
                          {p.yetkiler && Object.entries(p.yetkiler).some(([, v]) => v)
                            ? Object.entries(p.yetkiler).map(([k, v]) => v ? (
                                <span key={k} className="bg-orange-50 text-orange-600 border border-orange-100 text-[10px] px-2 py-0.5 rounded-full">{PERSONEL_YETKI_TANIM[k]?.label || k}</span>
                              ) : null)
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-5 border-b border-slate-100 sticky top-0 bg-white z-10 rounded-t-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800">{detayModal.full_name || 'İsimsiz Kullanıcı'}</p>
                  <p className="text-xs text-slate-400 font-mono">{detayModal.id}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {!detayDuzenle && (
                  <button onClick={() => setDetayDuzenle(true)} className={btnS + ' flex items-center gap-1 text-xs py-1.5'}>
                ...
                {detayDuzenle ? (
                  <div className="flex gap-3">
                    <button onClick={() => setDetayModal({ ...detayModal, aktif: true })}
                      className={'px-4 py-2 rounded-lg border text-sm font-medium transition ' + (detayModal.aktif !== false ? 'bg-green-500 text-white border-green-500' : 'text-slate-500 border-slate-200 bg-white')}>
                      ✓ Aktif
                    </button>
                    <button onClick={() => setDetayModal({ ...detayModal, aktif: false })}
                      className={'px-4 py-2 rounded-lg border text-sm font-medium transition ' + (detayModal.aktif === false ? 'bg-red-500 text-white border-red-500' : 'text-slate-500 border-slate-200 bg-white')}>
                      ✕ Pasif (Hesabı kapat)
                    </button>
                  </div>
                ) : (
                ...
                <div>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4">
                    <div className="mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-sm bg-white border border-slate-200 px-3 py-1.5 rounded-lg text-slate-700">
                          {detaySifreGoster ? (detayModal.sifre_acik || '(kayıtlı değil)') : '••••••••'}
                        </span>
                        <button onClick={() => setDetaySifreGoster(p => !p)}
                          className="p-1.5 text-slate-400 hover:text-slate-600 bg-white border border-slate-200 rounded-lg hover:border-slate-300 transition">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {Object.entries(KISITLAMA_TANIM).map(([key, tanim]) => {
                    ...
                    return (
                      <label key={key} className={'flex items-center gap-3 p-3 border rounded-xl transition-colors ' +
                        (detayDuzenle ? 'cursor-pointer ' : 'cursor-default ') +
                        (aktif ? 'border-red-200 bg-red-50' : 'border-slate-100 bg-slate-50/50')}>
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className={'text-xs font-medium truncate ' + (aktif ? 'text-red-700' : 'text-slate-600')}>{tanim.label}</p>
            ...
            {/* Modal Alt Butonlar */}
            <div className="flex items-center justify-between p-5 border-t border-slate-100 bg-slate-50 rounded-b-2xl sticky bottom-0">
              <button onClick={() => kullaniciSil(detayModal.id)} className={btnR + ' flex items-center gap-1.5 text-xs'}>
              ...
              {detayDuzenle && (
                <div className="flex gap-2">
                  <button onClick={() => { setDetayDuzenle(false); setDetaySifre(''); setDetayKisitlamalar(detayModal.kisitlamalar || {}); }}
                    className={btnS + ' flex items-center gap-1 text-xs py-1.5'}><X size={13} />İptal</button>
                  <button onClick={detayKaydet} disabled={detayKaydediyor}
                    className={btnO + ' flex items-center gap-1 text-xs disabled:opacity-60'}>
